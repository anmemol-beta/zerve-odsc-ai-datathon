...
export default function BlockDetail({
  ...
  return (
    <div className="absolute right-0 top-0 z-20 flex h-full w-full max-w-[440px] flex-col border-l border-slate-700 bg-slate-900/60 backdrop-blur-md">
      ...
      <footer className="border-t border-slate-700 p-4">
        <button
          ...
          className="w-full rounded-md border border-cyan-400/40 bg-cyan-500/15 px-3 py-2 text-xs font-medium text-cyan-200 transition hover:bg-cyan-500/15"
        >
          ↻ Re-pull from canvas
        </button>
      </footer>
  ...
  return (
    <div className="space-y-3">
      {Object.entries(vars).map(([slot, value]) => (
        ...
        >
          <div className="border-b border-slate-700 px-3 py-1.5 text-[10px] font-mono uppercase tracking-wider text-slate-400">
            {slot}
          </div>
...
function Skeleton() {
  return (
    <div className="h-24 animate-pulse rounded-md border border-slate-700 bg-slate-900/60" />
  );
}

function ErrorBanner({ error }: { error: unknown }) {
  return (
    <div className="rounded-md border border-rose-500/40 bg-rose-500/15 p-3 text-[11px] text-rose-200">
