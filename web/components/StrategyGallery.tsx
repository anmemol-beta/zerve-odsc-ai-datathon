...
const CHANNEL_COLOR: Record<string, string> = {
  email:             "text-cyan-300 border-cyan-400/40 bg-cyan-500/10",
  in_app_modal:      "text-pink-300 border-pink-400/40 bg-pink-500/10",
  sales_call:        "text-amber-300 border-amber-400/40 bg-amber-500/10",
  push_notification: "text-violet-300 border-violet-400/40 bg-violet-500/10",
  ad_retargeting:    "text-emerald-300 border-emerald-400/40 bg-emerald-500/10",
  lifecycle_drip:    "text-blue-300 border-blue-400/40 bg-blue-500/10",
};
            ...
            >
              ...
              <span
                className={`flex-shrink-0 rounded-full px-1.5 py-0.5 text-[9px] tabular-nums ${
                  lift > 1 ? "bg-emerald-500/20 text-emerald-300" : "bg-slate-800 text-slate-400"
                }`}
              >
                {lift.toFixed(1)}×
              </span>
...
function ActionCard({ action }: { action: StrategyAction }) {
  const channelClass = action.channel
    ? CHANNEL_COLOR[action.channel] ?? "text-slate-300 border-slate-700 bg-slate-800/40"
