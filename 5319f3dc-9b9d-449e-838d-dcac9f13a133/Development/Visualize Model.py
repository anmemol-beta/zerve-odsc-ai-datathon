...
for name, scores, color in [("logistic_reg", lr_proba_test, "#4c72b0"),
    ...
    axes[0, 0].plot(fpr, tpr, label=f"{name}  AUC={auc:.3f}", color=color, linewidth=1.5)
axes[0, 0].plot([0, 1], [0, 1], color="grey", linestyle=":", linewidth=1)
...
for name, scores, color in [("logistic_reg", lr_proba_test, "#4c72b0"),
    ...
    axes[0, 1].plot(rec, prec, label=f"{name}  AP={ap:.3f}", color=color, linewidth=1.5)
axes[0, 1].axhline(base_rate, color="grey", linestyle=":", linewidth=1, label=f"base rate {base_rate:.3f}")
axes[0, 1].set_xlabel("recall"); axes[0, 1].set_ylabel("precision")
axes[0, 1].set_title("Precision-Recall (preferred under imbalance)")
axes[0, 1].legend(loc="upper right", fontsize=9)
axes[0, 1].grid(True, alpha=0.3)

# LGBM feature importance
top_lgbm = lgbm_importance.head(15)
axes[1, 0].barh(range(len(top_lgbm)), top_lgbm.values, color="#dd8452")
axes[1, 0].set_yticks(range(len(top_lgbm)))
axes[1, 0].set_yticklabels(top_lgbm.index, fontsize=8)
axes[1, 0].invert_yaxis()
axes[1, 0].set_xlabel("gain")
axes[1, 0].set_title("LightGBM feature importance (top 15)")

# SHAP mean |value|
mean_abs_shap = pd.Series(np.abs(shap_values).mean(axis=0), index=feature_cols).sort_values(ascending=False).head(15)
axes[1, 1].barh(range(len(mean_abs_shap)), mean_abs_shap.values, color="#55a868")
axes[1, 1].set_yticks(range(len(mean_abs_shap)))
axes[1, 1].set_yticklabels(mean_abs_shap.index, fontsize=8)
axes[1, 1].invert_yaxis()
axes[1, 1].set_xlabel("mean |SHAP|")
axes[1, 1].set_title("SHAP impact (top 15)")
