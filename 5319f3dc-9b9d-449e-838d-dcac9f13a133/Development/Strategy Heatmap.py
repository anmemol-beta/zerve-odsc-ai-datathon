...
sh_rows = []
for sh_seg in strategies["segments"]:
    if not sh_seg.get("strategy") or not sh_seg["strategy"].get("actions"):
        continue
    for a in sh_seg["strategy"]["actions"]:
        sh_rows.append({
            "segment": sh_seg["label"],
            "segment_rank": sh_seg["stats"].get("stage_rank", 99),
...
sh_im = sh_ax.imshow(data, aspect="auto", cmap="RdPu",
...
for sh_i in range(data.shape[0]):
    for j in range(data.shape[1]):
        v = data[sh_i, j]
        if v > 0:
            sh_ax.text(j, sh_i, f"{v:.0f}",
...
plt.colorbar(sh_im, ax=sh_ax, label="ROI multiple")
...
for sh_i in range(data2.shape[0]):
    for j in range(data2.shape[1]):
        v = int(data2[sh_i, j])
        if v > 0:
            sh_ax.text(j, sh_i, str(v), ha="center", va="center", fontsize=9,
